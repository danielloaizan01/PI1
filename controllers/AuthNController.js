...
const show = (req, res, next) => {
  const token = req.headers.authorization; // Obtener el token de autenticación del encabezado
  if (!token) {
  ...
  jwt.verify(token, 'AzQ,PI)0(', (err, decoded) => {
    ...
    const usuarioID = decoded.userID; // Obtener el ID del usuario decodificado del token
    UserN.findById(usuarioID).findById(usuarioID)
      .then(response => {
        if (!response) {
